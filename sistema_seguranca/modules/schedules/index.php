...
const SCHEDULE_CSRF = <?= json_encode(csrf_token('_csrf')) ?>;
...
async function publishDay(btn, id){
  ...
  try{
    const r = await fetch('toggle_publish.php', {
      ...
      body: "id=" + encodeURIComponent(id) + "&_csrf=" + encodeURIComponent(SCHEDULE_CSRF)
    });
...
document.getElementById('btnGenGroups')?.addEventListener('click', async () => {
  ...
  try{
    const r = await fetch('generate_from_groups.php', {
      method: 'POST',
      headers: {'Content-Type': 'application/x-www-form-urlencoded'},
      body: 'month=' + encodeURIComponent(month) + '&_csrf=' + encodeURIComponent(SCHEDULE_CSRF)
    });
    const js = await r.json();
